 ...
 *
 */
class CList extends Component implements \IteratorAggregate
{
	/**
	 ...
	 * @throws \benben\base\Exception if the index is out of the range
	 */
	public function itemAt($index)
	{
		if(isset($this->_d[$index]))
			...
			throw new Exception(Benben::t('benben','List index "{index}" is out of bound.',
				array('{index}'=>$index)));
	}
	...
	public function insertAt($index,$item)
	{
		if(!$this->_r)
		{
			...
			else if($index>=0 && $index<$this->_c)
			{
				...
				throw new Exception(Benben::t('yii','List index "{index}" is out of bound.',
					array('{index}'=>$index)));
		}
		else
			throw new Exception(Benben::t('yii','The list is read only.'));
	}
	...
	public function removeAt($index)
	{
		if(!$this->_r)
		{
			...
			throw new Exception(Benben::t('benben','The list is read only.'));
	}
	...
	public function copyFrom($data)
	{
		if(is_array($data) || ($data instanceof \Traversable))
		{
			...
			throw new Exception(Benben::t('benben','List data must be an array or an object implementing Traversable.'));
	}
