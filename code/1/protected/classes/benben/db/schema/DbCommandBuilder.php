...
/**
 * DbCommandBuilder provides basic methods to create query commands for tables.
 *
 * @property DbConnection $dbConnection Database connection.
 * @property DbSchema $schema The schema for this command builder.
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @package system.db.schema
 * @since 1.0
 */
class DbCommandBuilder extends Component
{
	...
	/**
	 * @param DbSchema $schema the schema for this command builder
	 */
	...
	/**
	 * @return DbSchema the schema for this command builder.
	 */
	 ...
	 * @return DbCommand insert command
	 */
	 ...
	 * @param DbCriteria $criteria the query criteria
	 * @return DbCommand the created command
	...
	public function createUpdateCounterCommand($table,$counters,$criteria)
	{
		...
		if($fields!==array())
		{
			...
			throw new DbException(Benben::t('benben','No counter columns are being updated for table "{table}".',
				array('{table}'=>$table->name)));
	}
	 ...
	 * @throws CException if the condition is not string, array and DbCriteria
	 */
	public function createCriteria($condition='',$params=array())
	{
		if(is_array($condition))
			$criteria=new DbCriteria($condition);
		elseif($condition instanceof DbCriteria)
	 ...
	 * @param mixed $table table schema ({@link DbTableSchema}) or table name (string).
