...
/**
 * DbSchema is the base class for retrieving metadata information.
 *
 * @property DbConnection $dbConnection Database connection. The connection is active.
 * @property array $tables The metadata for all tables in the database.
 * Each array element is an instance of {@link DbTableSchema} (or its child class).
 * The array keys are table names.
 * @property array $tableNames All table names in the database.
 * @property DbCommandBuilder $commandBuilder The SQL command builder for this connection.
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
...
abstract class DbSchema extends Component
{
	...
	/**
	 * @return DbConnection database connection. The connection is active.
	 */
	...
	public function getTable($name,$refresh=false)
	{
		...
		else
		{
			...
			if(!isset($this->_cacheExclude[$name]) && ($duration=$this->_connection->schemaCachingDuration)>0 && $this->_connection->schemaCacheID!==false && ($cache=Benben::app()->getComponent($this->_connection->schemaCacheID))!==null)
			{
	...
	/**
	 * @return DbCommandBuilder the SQL command builder for this connection.
	 */
	...
	{
		if(($duration=$this->_connection->schemaCachingDuration)>0 && $this->_connection->schemaCacheID!==false && ($cache=Benben::app()->getComponent($this->_connection->schemaCacheID))!==null)
		{
	 ...
	 * @param DbTableSchema $table the table schema whose primary key sequence will be reset
	...
	{
		throw new DbException(Benben::t('yii','{class} does not support fetching all table names.',
