...
use benben\Benben;
/**
 * DateFormatter class file.
 *
 * @author Wei Zhuo <weizhuo[at]gmail[dot]com>
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @link http://www.Benbenframework.com/
 * @copyright Copyright &copy; 2008-2011 Benben Software LLC
 * @license http://www.Benbenframework.com/license/
 */

/**
 * DateFormatter provides date/time localization functionalities.
 *
 * DateFormatter allows you to format dates and times in a locale-sensitive manner.
 ...
 * The patterns that DateFormatter recognizes are as defined in
 * {@link http://www.unicode.org/reports/tr35/#Date_Format_Patterns CLDR}.
 *
 * DateFormatter supports predefined patterns as well as customized ones:
 * <ul>
...
class DateFormatter extends Component
{
	...
	/**
	 * Constructor.
	 * @param mixed $locale locale ID (string) or Locale instance
	 */
	 ...
	 * @return string formatted date time. Null if $time is null. (the null value check is available since Benben 1.1.11)
	 */
	 ...
	 * @param array $date result of {@link Timestamp::getdate}.
	...
	protected function formatMonth($pattern,$date)
	{
		...
		switch($pattern)
		{
			...
			default:
				throw new Exception(Benben::t('Benben','The pattern for month must be "M", "MM", "MMM", "MMMM", "L", "LL", "LLL" or "LLLL".'));
		}
	}
	...
	protected function formatDay($pattern,$date)
	{
		...
		if($pattern==='d')
			...
			throw new Exception(Benben::t('Benben','The pattern for day of the month must be "d" or "dd".'));
	}
	...
	protected function formatDayInYear($pattern,$date)
	{
		...
		if(($n=strlen($pattern))<=3)
			...
			throw new Exception(Benben::t('Benben','The pattern for day in year must be "D", "DD" or "DDD".'));
	}
	...
	protected function formatDayInMonth($pattern,$date)
	{
		if($pattern==='F')
			...
			throw new Exception(Benben::t('Benben','The pattern for day in month must be "F".'));
	}
	...
	protected function formatDayInWeek($pattern,$date)
	{
		...
		switch($pattern)
		{
			...
			default:
				throw new Exception(Benben::t('Benben','The pattern for day of the week must be "E", "EE", "EEE", "EEEE", "EEEEE", "e", "ee", "eee", "eeee", "eeeee", "c", "cccc" or "ccccc".'));
		}
	}
	...
	protected function formatPeriod($pattern,$date)
	{
		if($pattern==='a')
		{
			...
			throw new Exception(Benben::t('Benben','The pattern for AM/PM marker must be "a".'));
	}
	...
	protected function formatHour24($pattern,$date)
	{
		...
		if($pattern==='H')
			...
			throw new Exception(Benben::t('Benben','The pattern for 24 hour format must be "H" or "HH".'));
	}
	...
	protected function formatHour12($pattern,$date)
	{
		...
		if($pattern==='h')
			...
			throw new Exception(Benben::t('Benben','The pattern for 12 hour format must be "h" or "hh".'));
	}
	...
	protected function formatHourInDay($pattern,$date)
	{
		...
		if($pattern==='k')
			...
			throw new Exception(Benben::t('Benben','The pattern for hour in day must be "k" or "kk".'));
	}
	...
	protected function formatHourInPeriod($pattern,$date)
	{
		...
		if($pattern==='K')
			...
			throw new Exception(Benben::t('Benben','The pattern for hour in AM/PM must be "K" or "KK".'));
	}
	...
	protected function formatMinutes($pattern,$date)
	{
		...
		if($pattern==='m')
			...
			throw new Exception(Benben::t('Benben','The pattern for minutes must be "m" or "mm".'));
	}
	...
	protected function formatSeconds($pattern,$date)
	{
		...
		if($pattern==='s')
			...
			throw new Exception(Benben::t('Benben','The pattern for seconds must be "s" or "ss".'));
	}
	...
	protected function formatWeekInYear($pattern,$date)
	{
		if($pattern==='w')
			...
			throw new Exception(Benben::t('Benben','The pattern for week in year must be "w".'));
	}

	/**
	 * Get week in the month.
	 * @param array $pattern result of {@link Timestamp::getdate}.
	...
	protected function formatWeekInMonth($pattern,$date)
	{
		if($pattern==='W')
			...
			throw new Exception(Benben::t('Benben','The pattern for week in month must be "W".'));
	}
	...
	protected function formatTimeZone($pattern,$date)
	{
		...
		elseif($pattern[0]==='Z')
			...
			throw new Exception(Benben::t('Benben','The pattern for time zone must be "z" or "v".'));
	}
	...
	protected function formatEra($pattern,$date)
	{
		...
		switch($pattern)
		{
			...
			default:
				throw new Exception(Benben::t('Benben','The pattern for era must be "G", "GG", "GGG", "GGGG" or "GGGGG".'));
		}
	}
}
