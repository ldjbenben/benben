 ...
 * placeholder, recognized by all validators (see {@link CValidator}), StringValidator allows for the following
...
class StringValidator extends Validator
{
	...
	protected function validateAttribute($object,$attribute)
	{
		...
		if(function_exists('mb_strlen') && $this->encoding!==false)
			$length=mb_strlen($value, $this->encoding ? $this->encoding : Benben::app()->charset);
		else
			$length=strlen($value);

		if($this->min!==null && $length<$this->min)
		{
			$message=$this->tooShort!==null?$this->tooShort:Benben::t('benben','{attribute} is too short (minimum is {min} characters).');
			$this->addError($object,$attribute,$message,array('{min}'=>$this->min));
		}
		if($this->max!==null && $length>$this->max)
		{
			$message=$this->tooLong!==null?$this->tooLong:Benben::t('benben','{attribute} is too long (maximum is {max} characters).');
			$this->addError($object,$attribute,$message,array('{max}'=>$this->max));
		}
		if($this->is!==null && $length!==$this->is)
		{
			$message=$this->message!==null?$this->message:Benben::t('benben','{attribute} is of the wrong length (should be {length} characters).');
	...
	public function clientValidateAttribute($object,$attribute)
	{
		...
		if(($message=$this->message)===null)
			$message=Benben::t('benben','{attribute} is of the wrong length (should be {length} characters).');
		...
		if(($tooShort=$this->tooShort)===null)
			$tooShort=Benben::t('benben','{attribute} is too short (minimum is {min} characters).');
		...
		if(($tooLong=$this->tooLong)===null)
			$tooLong=Benben::t('benben','{attribute} is too long (maximum is {max} characters).');
