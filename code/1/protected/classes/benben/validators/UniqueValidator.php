...
class UniqueValidator extends Validator
{
	...
	protected function validateAttribute($object,$attribute)
	{
		...
		$className=$this->className===null?get_class($object):Benben::import($this->className);
		$attributeName=$this->attributeName===null?$attribute:$this->attributeName;
		$finder=ActiveRecord::model($className);
		$table=$finder->getTableSchema();
		if(($column=$table->getColumn($attributeName))===null)
			throw new Exception(Benben::t('benben','Table "{table}" does not have a column named "{column}".',
		...
		if($exists)
		{
			$message=$this->message!==null?$this->message:Benben::t('benben','{attribute} "{value}" has already been taken.');
