...
/**
 * AssetManager is a Web application component that manages private files (called assets) and makes them accessible by Web clients.
 *
 ...
 * @since 1.0
 */
class AssetManager extends ApplicationComponent
{
	/**
	...
	public function setBasePath($value)
	{
		if(($basePath=realpath($value))!==false && is_dir($basePath) && is_writable($basePath))
			...
			throw new Exception(Benben::t('benben','AssetManager.basePath "{path}" is invalid. Please make sure the directory exists and is writable by the Web server process.',
				array('{path}'=>$value)));
	}
	...
	public function publish($path,$hashByName=false,$level=-1,$forceCopy=null)
	{
		...
		if($forceCopy && $this->linkAssets)
			throw new Exception(Benben::t('benben','The "forceCopy" and "linkAssets" cannot be both true.'));
		...
		throw new Exception(Benben::t('benben','The asset "{asset}" to be published does not exist.',
			array('{asset}'=>$path)));
	}
	...
	{
		return sprintf('%x',crc32($path.Benben::getVersion()));
	}
