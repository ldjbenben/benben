...
/**
 * ClientScript manages JavaScript and CSS stylesheets for views.
 *
 ...
 * @since 1.0
 */
class ClientScript extends ApplicationComponent
{
	/**
	 ...
	 * that will be passed as the last parameter of {@link Html::metaTag}.
	 * @since 1.1.3
	 */
	protected $metaTags=array();
	/**
	 * @var array the registered head link tags. Each array element represents an option array
	 * that will be passed as the last parameter of {@link Html::linkTag}.
	 * @since 1.1.3
	 */
	 ...
	 * will be inserted in the page. This can be one of the ClientScript::POS_* constants.
	 * Defaults to ClientScript::POS_HEAD.
	 * @since 1.1.3
	 */
	 ...
	 * This can be one of the ClientScript::POS_* constants.
	 * Defaults to ClientScript::POS_HEAD.
	 * @since 1.1.11
	 */
	 ...
	 * This can be one of the ClientScript::POS_* constants.
	 * Defaults to ClientScript::POS_READY.
	 * @since 1.1.11
	 */
	 ...
	 * rendering content. ClientScript thus gets a chance to insert script tags
	...
	public function renderHead(&$output)
	{
		...
		foreach($this->linkTags as $link)
			$html.=Html::linkTag(null,null,null,null,$link)."\n";
		foreach($this->cssFiles as $url=>$media)
	...
	public function getCoreScriptUrl()
	{
		if($this->_baseUrl!==null)
			...
			return $this->_baseUrl=Benben::app()->getAssetManager()->publish(BENBEN_PATH.'/web/js/source');
	}
	...
	public function getPackageBaseUrl($name)
	{
		...
		if(isset($package['baseUrl']))
		{
			...
			if($baseUrl==='' || $baseUrl[0]!=='/' && strpos($baseUrl,'://')===false)
				$baseUrl=Benben::app()->getRequest()->getBaseUrl().'/'.$baseUrl;
			$baseUrl=rtrim($baseUrl,'/');
		}
		elseif(isset($package['basePath']))
			$baseUrl=Benben::app()->getAssetManager()->publish(Benben::getPathOfAlias($package['basePath']));
		else
	...
	public function registerCoreScript($name)
	{
		...
		else
		{
			if($this->corePackages===null)
				$this->corePackages=require(BENBEN_PATH.'/web/js/packages.php');
	 ...
	 * <li>ClientScript::POS_BEGIN : the script is inserted at the beginning of the body section.</li>
	 * <li>ClientScript::POS_END : the script is inserted at the end of the body section.</li>
	 * <li>ClientScript::POS_LOAD : the script is inserted in the window.onload() function.</li>
	 * <li>ClientScript::POS_READY : the script is inserted in the jQuery's ready function.</li>
	 * </ul>
	 * @return ClientScript the ClientScript object itself (to support method chaining, available since version 1.1.5).
	 */
	 ...
	 * This is a shortcut to Benben::app()->controller->recordCachingAction.
	...
	{
		if(($controller=Benben::app()->getController())!==null)
	 ...
	 * @see ClientScript::packages.
	 * @return ClientScript the ClientScript object itself (to support method chaining, available since version 1.1.10).
	 *
	 * @since 1.1.9
	 */
