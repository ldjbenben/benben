 ...
 * @since 1.0
 */
class HttpSession extends ApplicationComponent
{
	/**
	...
	public function open()
	{
		...
		@session_start();
		if(BENBEN_DEBUG && session_id()=='')
		{
			$message=Benben::t('benben','Failed to start session.');
			...
			Benben::log($message, Logger::LEVEL_WARNING, 'system.web.HttpSession');
		}
	}
	...
	public function setSavePath($value)
	{
		if(is_dir($value))
			...
			throw new Exception(Benben::t('benben','HttpSession.savePath "{path}" is not a valid directory.',
				array('{path}'=>$value)));
	}
	...
	public function setCookieMode($value)
	{
		...
		else if($value==='only')
		{
			...
			throw new Exception(Benben::t('benben','HttpSession.cookieMode can only be "none", "allow" or "only".'));
	}
	...
	public function setGCProbability($value)
	{
		...
		if($value>=0 && $value<=100)
		{
			...
			throw new Exception(Benben::t('benben','HttpSession.gcProbability "{value}" is invalid. It must be an integer between 0 and 100.',
				array('{value}'=>$value)));
	}
