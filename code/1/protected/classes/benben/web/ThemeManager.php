...
class ThemeManager extends ApplicationComponent
{
	...
	public function getTheme($name)
	{
		...
		if(is_dir($themePath))
		{
			$class=Benben::import($this->themeClass, true);
	...
	public function getBasePath()
	{
		if($this->_basePath===null)
			$this->setBasePath(dirname(Benben::app()->getRequest()->getScriptFile()).DIRECTORY_SEPARATOR.self::DEFAULT_BASEPATH);
		return $this->_basePath;
	}

	/**
	 * @param string $value the base path for all themes.
	 * @throws Exception if the base path does not exist
	 */
	public function setBasePath($value)
	{
		$this->_basePath=realpath($value);
		if($this->_basePath===false || !is_dir($this->_basePath))
			throw new Exception(Benben::t('Benben','Theme directory "{directory}" does not exist.',array('{directory}'=>$value)));
	}
	...
	public function getBaseUrl()
	{
		if($this->_baseUrl===null)
			$this->_baseUrl=Benben::app()->getBaseUrl().'/'.self::DEFAULT_BASEPATH;
