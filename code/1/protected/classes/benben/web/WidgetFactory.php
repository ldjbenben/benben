...
/**
 * WidgetFactory creates new widgets to be used in views.
 *
 * WidgetFactory is used as the default "widgetFactory" application component.
 *
 ...
 * </pre>
 *
 * WidgetFactory implements the "skin" feature, which allows a new widget to be created
 * and initialized with a set of predefined property values (called skin).
 *
 * When WidgetFactory is used to create a new widget, it will first instantiate the
 ...
 * to be "short".
 *
 * By default, WidgetFactory looks for the skin of a widget under the "skins" directory
 ...
 * @since 1.1
 */
class WidgetFactory extends ApplicationComponent
{
	/**
	...
	public function init()
	{
		...
		if($this->enableSkin && $this->skinPath===null)
			$this->skinPath=Benben::app()->getViewPath().DIRECTORY_SEPARATOR.'skins';
	}
	...
	{
		$className=Benben::import($className,true);
	...
	protected function getSkin($className,$skinName)
	{
		if(!isset($this->_skins[$className][$skinName]))
		{
			...
			if(($theme=Benben::app()->getTheme())!==null)
			{
