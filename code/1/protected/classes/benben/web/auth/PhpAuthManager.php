...
use benben\Benben;
use benben\web\auth\AuthAssignment;
use benben\web\auth\AuthItem;
...
/**
 * PhpAuthManager represents an authorization manager that stores authorization information in terms of a PHP script file.
 *
 ...
 *
 * PhpAuthManager is mainly suitable for authorization data that is not too big
 ...
 * @since 1.0
 */
class PhpAuthManager extends AuthManager
{
	/**
	...
	public function init()
	{
		...
		if($this->authFile===null)
			$this->authFile=Benben::getPathOfAlias('application.data.auth').'.php';
		$this->load();
	}
	...
	public function checkAccess($itemName,$userId,$params=array())
	{
		...
		Benben::trace('Checking permission "'.$item->getName().'"','benben.web.auth.PhpAuthManager');
	...
	public function addItemChild($itemName,$childName)
	{
		if(!isset($this->_items[$childName],$this->_items[$itemName]))
			throw new Exception(Benben::t('benben','Either "{parent}" or "{child}" does not exist.',array('{child}'=>$childName,'{name}'=>$itemName)));
		$child=$this->_items[$childName];
		$item=$this->_items[$itemName];
		$this->checkItemChildType($item->getType(),$child->getType());
		if($this->detectLoop($itemName,$childName))
			throw new Exception(Benben::t('benben','Cannot add "{child}" as a child of "{parent}". A loop has been detected.',
				array('{child}'=>$childName,'{parent}'=>$itemName)));
		if(isset($this->_children[$itemName][$childName]))
			throw new Exception(Benben::t('benben','The item "{parent}" already has a child "{child}".',
				array('{child}'=>$childName,'{parent}'=>$itemName)));
	...
	{
		if(!isset($this->_items[$itemName]))
			throw new Exception(Benben::t('benben','Unknown authorization item "{name}".',array('{name}'=>$itemName)));
		else if(isset($this->_assignments[$userId][$itemName]))
			throw new Exception(Benben::t('benben','Authorization item "{item}" has already been assigned to user "{user}".',
				array('{item}'=>$itemName,'{user}'=>$userId)));
		else
			return $this->_assignments[$userId][$itemName]=new AuthAssignment($this,$itemName,$userId,$bizRule,$data);
	}
	...
	public function createAuthItem($name,$type,$description='',$bizRule=null,$data=null)
	{
		if(isset($this->_items[$name]))
			throw new Exception(Benben::t('benben','Unable to add an item whose name is the same as an existing item.'));
		return $this->_items[$name]=new AuthItem($this,$name,$type,$description,$bizRule,$data);
	}
	...
	public function saveAuthItem($item,$oldName=null)
	{
		if($oldName!==null && ($newName=$item->getName())!==$oldName) // name changed
		{
			if(isset($this->_items[$newName]))
				throw new Exception(Benben::t('benben','Unable to change the item name. The name "{name}" is already used by another item.',array('{name}'=>$newName)));
	...
	public function load()
	{
		...
		foreach($items as $name=>$item)
			$this->_items[$name]=new AuthItem($this,$name,$item['type'],$item['description'],$item['bizRule'],$item['data']);
		...
		foreach($items as $name=>$item)
		{
			...
			if(isset($item['assignments']))
			{
				...
				{
					$this->_assignments[$userId][$name]=new AuthAssignment($this,$name,$userId,$assignment['bizRule'],$assignment['data']);
				}
			}
		}
	}
