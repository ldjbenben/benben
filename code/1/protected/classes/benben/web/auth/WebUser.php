 ...
 *
 * WebUser is used as an application component whose ID is 'user'.
 * Therefore, at any place one can access the user state via
 * <code>Benben::app()->user</code>.
 *
 * WebUser should be used together with an {@link IUserIdentity identity}
 * which implements the actual authentication algorithm.
 *
 * A typical authentication process using WebUser is as follows:
 * <ol>
 ...
 * <li>If valid, call {@link WebUser::login} to login the user, and
...
{
	const FLASH_KEY_PREFIX='benben.WebUser.flash.';
	const FLASH_COUNTERS='benben.WebUser.flashcounters';
	...
	*/
		...
		if($this->beforeLogin($id,$states,false))
		{
			...
			if($duration>0)
			{
				if($this->allowAutoLogin)
					...
					throw new Exception(Benben::t('benben','{class}.allowAutoLogin must be set true in order to use cookie-based authentication.',
	...
	public function logout($destroySession=true)
	{
		if($this->beforeLogout())
		{
			if($this->allowAutoLogin)
			{
				Benben::app()->getRequest()->getCookies()->remove($this->getStateKeyPrefix());
				if($this->identityCookie!==null)
				{
					$cookie=$this->createIdentityCookie($this->getStateKeyPrefix());
					$cookie->value=null;
					$cookie->expire=0;
					Benben::app()->getRequest()->getCookies()->add($cookie->name,$cookie);
				}
			}
			if($destroySession)
				Benben::app()->getSession()->destroy();
			else
				$this->clearStates();
			$this->afterLogout();
		}
	...
	{
		return $this->getState('__returnUrl', $defaultUrl===null ? Benben::app()->getRequest()->getScriptUrl() : Html::normalizeUrl($defaultUrl));
	}
	...
	public function loginRequired()
	{
		...
		if(($url=$this->loginUrl)!==null)
		{
			...
			throw new HttpException(403,Benben::t('benben','Login Required'));
	}
	...
	protected function renewCookie()
	{
		...
		if($cookie && !empty($cookie->value) && ($data=Benben::app()->getSecurityManager()->validateData($cookie->value))!==false)
		{
			$data=@unserialize($data);
			if(is_array($data) && isset($data[0],$data[1],$data[2],$data[3]))
			{
	...
	public function getStateKeyPrefix()
	{
		if($this->_keyPrefix!==null)
			...
			return $this->_keyPrefix=md5('benben.'.get_class($this).'.'.Benben::app()->getId());
	}
	 ...
	 *
	 * This function is designed to be used by WebUser descendant classes
	...
	{
		Benben::app()->getSession()->regenerateID();
		$this->setId($id);
		$this->setName($name);
	...
	public function checkAccess($operation,$params=array(),$allowCaching=true)
	{
		...
		$access=Benben::app()->getAuthManager()->checkAccess($operation,$this->getId(),$params);
