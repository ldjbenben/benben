<?php
namespace benben\web\helpers;

use benben\Benben;
use benben\web\Controller;
use benben\validators\StringValidator;
...
class Html
{
	...
	{
		return htmlspecialchars($text,ENT_QUOTES,Benben::app()->charset);
	}
	...
	public static function encodeArray($data)
	{
		...
		{
			if(is_string($key))
				$key=htmlspecialchars($key,ENT_QUOTES,Benben::app()->charset);
			if(is_string($value))
				$value=htmlspecialchars($value,ENT_QUOTES,Benben::app()->charset);
	...
	$content="$seconds";
		if($url!=='')
			$content.=';'.self::normalizeUrl($url);
					Benben::app()->clientScript->registerMetaTag($content,null,'refresh');
	}
	...
	*/
	...
	/**
					...
					*/
					 ...
					 * with CSS class 'required' (customizable with Html::$requiredCss),
					 * and be decorated with {@link Html::beforeRequiredLabel} and
					 * {@link Html::afterRequiredLabel}.</li>
					 * </ul>
...
EOD;
					...
					{
						Benben::app()->getClientScript()->registerCoreScript('jquery');
					...
					{
						return Benben::app()->getAssetManager()->publish($path,$hashByName);
					}
					...
					public static function normalizeUrl($url)
					{
						if(is_array($url))
						{
							if(isset($url[0]))
							{
								if(($c=Benben::app()->getController())!==null)
									$url=$c->createUrl($url[0],array_splice($url,1));
								else
									$url=Benben::app()->createUrl($url[0],array_splice($url,1));
							}
							else
								$url='';
						}
						return $url==='' ? Benben::app()->getRequest()->getUrl() : $url;
					}
					 ...
					 * the input for the attribute (see {@link Model::getAttributeLabel}.
					 ...
					 * {@link Model::getAttributeLabel} will be called to get the label for display.
					 ...
					 * In particular, it calls {@link Model::isAttributeRequired} to determine
					 * if the attribute is required.
					 * If so, it will add a CSS class {@link Html::requiredCss} to the label,
					 * and decorate the label with {@link Html::beforeRequiredLabel} and
					 * {@link Html::afterRequiredLabel}.
					 * @param Model $model the data model
					 * @param string $attribute the attribute
					 * @param array $htmlOptions additional HTML attributes.
					 * @return string the generated label tag
					...
					public static function errorSummary($model,$header=null,$footer=null,$htmlOptions=array())
					{
						...
						{
							if($header===null)
								$header='<p>'.Benben::t('yii','Please fix the following input errors:').'</p>';
							if(!isset($htmlOptions['class']))
					...
					protected static function clientChange($event,&$htmlOptions)
					{
						...
						if(isset($htmlOptions['submit']))
						{
							...
							$request=Benben::app()->getRequest();
							if($request->enableCsrfValidation && isset($htmlOptions['csrf']) && $htmlOptions['csrf'])
								$htmlOptions['params'][$request->csrfTokenName]=$request->getCsrfToken();
							if(isset($htmlOptions['params']))
								$params=JavaScript::encode($htmlOptions['params']);
							else
								$params='{}';
							if($htmlOptions['submit']!=='')
								$url=JavaScript::quote(self::normalizeUrl($htmlOptions['submit']));
							else
								$url='';
							$handler.="jQuery.Benben.submitForm(this,'$url',$params);{$return};";
						}
						...
						if($live)
							$cs->registerScript('Benben.Html.#' . $id, "$('body').on('$event','#$id',function(){{$handler}});");
						else
							$cs->registerScript('Benben.Html.#' . $id, "$('#$id').on('$event', function(){{$handler}});");
					...
					{
						$model_name = ClassHelper::getModelLastName($model);
     ...
     * @param unknown_type $closeTag
     * @return string
     */
  /*   static public function tag($tagName, $htmlOptions = array(), $content=false, $closeTag = true)
    {
        $selfCloseTag = array('input','img');
        $html = "<{$tagName}";
        if (is_array($htmlOptions) && !empty($htmlOptions))
        {
            foreach ($htmlOptions as $n=>$v)
            {
                $html .= " {$n}=\"{$v}\"";
            }
        }
