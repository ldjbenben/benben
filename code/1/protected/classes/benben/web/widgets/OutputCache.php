...
/**
 * OutputCache enables caching the output generated by an action or a view fragment.
 *
 ...
 * the original output.
 *
 * Since OutputCache extends from {@link CFilterWidget}, it can be used
 ...
 * </pre>
 *
 * OutputCache must work with a cache application component specified via {@link cacheID}.
 * If the cache application component is not available, OutputCache will be disabled.
 *
 ...
 * some parameters. OutputCache supports four kinds of variations:
 * <ul>
 ...
 * @since 1.0
 */
class OutputCache extends FilterWidget
{
	/**
	 * Prefix to the keys for storing cached data
	 */
	const CACHE_KEY_PREFIX='Benben.OutputCache.';
	...
	public function run()
	{
		...
		elseif($this->_cache!==null)
		{
			...
			if(is_array($this->dependency))
				$this->dependency=Benben::createComponent($this->dependency);
	...
	{
		if((empty($this->requestTypes) || in_array(Benben::app()->getRequest()->getRequestType(),$this->requestTypes))
	...
	{
		return Benben::app()->getComponent($this->cacheID);
	}
	...
	protected function getCacheKey()
	{
		...
		else
		{
			...
			if($this->varyBySession)
				$key.=Benben::app()->getSession()->getSessionID();
			$key.='.';
	...
	protected function replayActions()
	{
		...
		foreach($this->_actions as $action)
		{
			...
			elseif($action[0]==='' && function_exists($action[1]))
				...
				throw new Exception(Benben::t('benben','Unable to replay the action "{object}.{method}". The method does not exist.',
